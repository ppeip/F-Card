...
public class PersonalCardServiceImpl implements PersonalCardService {
    ...
    @Transactional
    @Override
    public boolean rollCard(User user) {
        ...
        if (influence == 0) {
            try {
                ...
                cardInHandDao.insertCardInHand(new CardInHand(card.getAbility1(),card.getAbility2(),
                        card.getAbility3(),card.getAbility4(),card.getAbility5(),card.getCardLevel(),
                        card.getExperience(),card.getIntroduction(),card.getPreferance(),user.getUID(),

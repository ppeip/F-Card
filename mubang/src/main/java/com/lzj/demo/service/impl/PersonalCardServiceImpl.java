...
public class PersonalCardServiceImpl implements PersonalCardService {
    ...
    @Override
    public List<PersonalCard> queryCardInHandByUID(String UID) {
        return personalCardDao.queryCardInHandByUID(UID);
    }
    ...
    @Transactional
    @Override
    public User updatePersonalCardByUid(User user, int num) {
        if (user.getUID() != null && !"".equals(user.getUID())) {
            try {
                user = userDao.queryUserByUid(user.getUID());
                int max = user.getTowerNumber();
                int effectedNum1;
                if (max - 9 <= num && num != max+1) {
                    effectedNum1 = personalCardDao.updatePersonalCard5(user.getUID());
                }else if(num == max+1){
                    effectedNum1 = personalCardDao.updatePersonalCard10(user.getUID());
                    user.setCoin(user.getCoin()+100+20*num);
                    user.setTowerNumber(num);
                    user.setWin(user.getWin()+1);
                    user.setBattle(user.getBattle()+1);
                    user.setCollegeInfluence(user.getCollegeInfluence()+num*100);
                    userDao.updateUser(user);
                }else{
                    effectedNum1 = personalCardDao.updatePersonalCard2(user.getUID());
                }
                if (effectedNum1 > 0) {
                    return user;
                } else {
    ...
    public PersonalCard rollCard(User user) {
        ...
        if (influence != 0) {
            try {
                ...
                double plus = 1.0 * user.getCollegeInfluence() / 10000000;
                //抽卡逻辑
                if (x < 0.01 + plus * 3) {
                    cardList = cardDao.queryCardByRank("院士");
                } else if (x < 0.05 + plus * 10) {
                    cardList = cardDao.queryCardByRank("教授");
                } else if (x < 0.2 + plus * 80) {
